...
public sealed class FavoriteService : IFavoriteService
{
    ...
    public async Task<Result<IReadOnlyList<FavoriteDto>>> GetFavoritesAsync(
        int userId,
        CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            return Result<IReadOnlyList<FavoriteDto>>.Ok(
                _mapper.Map<IReadOnlyList<FavoriteDto>>(favorites));
        }
        catch
        {
            throw;
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            if (await _favoriteRepository.ExistsAsync(
                    dto.UserId,
                    dto.ProductId,
                    cancellationToken))
            {
                throw new ConflictException(
                    "This product is already in the user's favorites.");
            }
            ...
            var result = new FavoriteDto
            {
                ...
                ProductImageUrl = product.ProductImages
                    .FirstOrDefault()
                    ?.ImageUrl,
                CreatedDate = entity.CreatedDate
            };

            return Result<FavoriteDto>.Ok(
                result,
                "Favorite added.");
        }
        catch
        ...
        try
        {
                ...
                favorite.UserId != userId.Value)
            {
                throw new UnauthorizedException(
                    "You cannot remove another user's favorite.");
            }
